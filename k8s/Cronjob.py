...
class Cronjob:
    ...
    def get_cronjob_info(self):
        ...
        logger.info("获取cronjob信息完成")
        return cronjob_info

    def create_cronjob_yaml(self, cronjob_info):
        logger = Logger("server")
        logger.info("开始创建cronjob.yaml")
        logger.info("cronjob配置如下：")
        logger.info(cronjob_info)
        cronjob_yaml_j2 = '%s/templates/k8s/cronjob.yaml.j2' % sys.path[0]
        cronjob_yaml = '%s/cronjob.yaml' % self.k8s_path

        j2_to_file("server", cronjob_info, cronjob_yaml_j2, cronjob_yaml)
        logger.info("cronjob.yaml已生成。")
        apply_command_list = ["kubectl apply -f %s" % cronjob_yaml]
